...
# include <cerrno>
# include <arpa/inet.h>
# include <cstdlib> // CAMBIOS
# include <algorithm> // CAMBIOS
# include <ctime> // CAMBIOS
# include <cstdio> // CAMBIOS
...
Server::~Server()
{
    ...
    for (std::map<std::string, Channel*>::iterator it = _channels.begin();
        ...
        delete it->second;
#ifdef BONUS // CAMBIOS
    for (std::map<int, FileTransfer*>::iterator it = _transfers.begin(); // CAMBIOS
         it != _transfers.end(); ++it) // CAMBIOS
        delete it->second; // CAMBIOS
    _transfers.clear(); // CAMBIOS
#endif // CAMBIOS
    if (_serverFd != -1)
        close(_serverFd);
}
...
void Server::_cleanEmptyChannels()
{
    ...
    while (it != _channels.end())
    {
        ...
        {
            delete it->second;
            _channels.erase(it++); // CAMBIOS (fix C++98)
        }
        else
            ++it;
    }
...
#ifdef BONUS // CAMBIOS
    else if (command == "DCC")     _cmdDcc(client, params, trailing); // CAMBIOS
#endif // CAMBIOS
    else
...
// ───────────────────────────────────────────── // CAMBIOS
 // CAMBIOS
void Server::_cmdDcc(Client& client, const std::vector<std::string>& params, // CAMBIOS
                     const std::string& trailing) // CAMBIOS
{ // CAMBIOS
    if (params.empty()) // CAMBIOS
    { // CAMBIOS
        _sendReply(client.getFd(), "461", client.getNick(), // CAMBIOS
                   "DCC :Not enough parameters"); // CAMBIOS
        return; // CAMBIOS
    } // CAMBIOS
 // CAMBIOS
    (void)trailing; // CAMBIOS
    const std::string& subcmd = params[0]; // CAMBIOS
 // CAMBIOS
    if (subcmd == "SEND") // CAMBIOS
        _dccSend(client, params); // CAMBIOS
    else if (subcmd == "ACCEPT") // CAMBIOS
        _dccAccept(client, params); // CAMBIOS
    else if (subcmd == "DENY") // CAMBIOS
        _dccDeny(client, params); // CAMBIOS
    else if (subcmd == "CANCEL") // CAMBIOS
        _dccCancel(client, params); // CAMBIOS
    else if (subcmd == "LIST") // CAMBIOS
        _dccList(client); // CAMBIOS
    else // CAMBIOS
        _sendReply(client.getFd(), "461", client.getNick(), // CAMBIOS
                   "DCC :Unknown subcommand (try SEND, ACCEPT, DENY, CANCEL, LIST)"); // CAMBIOS
} // CAMBIOS
 // CAMBIOS
void Server::_dccSend(Client& client, const std::vector<std::string>& params) // CAMBIOS
{ // CAMBIOS
    // DCC SEND <target> <filepath> // CAMBIOS
    if (params.size() < 3) // CAMBIOS
    { // CAMBIOS
        _sendReply(client.getFd(), "461", client.getNick(), // CAMBIOS
                   "DCC SEND :Not enough parameters (DCC SEND <nick> <filepath>)"); // CAMBIOS
        return; // CAMBIOS
    } // CAMBIOS
 // CAMBIOS
    const std::string& targetNick = params[1]; // CAMBIOS
    const std::string& filepath   = params[2]; // CAMBIOS
 // CAMBIOS
    Client* target = _getClientByNick(targetNick); // CAMBIOS
    if (target == NULL) // CAMBIOS
    { // CAMBIOS
        _sendReply(client.getFd(), "401", client.getNick(), // CAMBIOS
                   targetNick + " :No such nick"); // CAMBIOS
        return; // CAMBIOS
    } // CAMBIOS
 // CAMBIOS
    // Extraer solo el nombre del archivo de la ruta // CAMBIOS
    std::string filename = filepath; // CAMBIOS
    size_t slashPos = filename.find_last_of("/\\"); // CAMBIOS
    if (slashPos != std::string::npos) // CAMBIOS
        filename = filename.substr(slashPos + 1); // CAMBIOS
 // CAMBIOS
    // Crear la transferencia // CAMBIOS
    int id = _nextTransferId++; // CAMBIOS
    FileTransfer* ft = new FileTransfer(id, &client, target, filename, filepath); // CAMBIOS
    _transfers[id] = ft; // CAMBIOS
 // CAMBIOS
    // Verificar que el archivo exista // CAMBIOS
    if (ft->getFileSize() <= 0) // CAMBIOS
    { // CAMBIOS
        _sendReply(client.getFd(), "554", client.getNick(), // CAMBIOS
                   "DCC :File not found or empty: " + filepath); // CAMBIOS
        _transfers.erase(id); // CAMBIOS
        delete ft; // CAMBIOS
        return; // CAMBIOS
    } // CAMBIOS
 // CAMBIOS
    // Confirmar al sender // CAMBIOS
    _sendReply(client.getFd(), "554", client.getNick(), // CAMBIOS
               "DCC :Transfer #" + toStr(id) // CAMBIOS
               + " for \"" + filename + "\" (" + toStr(ft->getFileSize()) // CAMBIOS
               + " bytes) offered to " + targetNick); // CAMBIOS
 // CAMBIOS
    // Notificar al receiver // CAMBIOS
    _send(target->getFd(), ":" + _name + " NOTICE " + target->getNick() // CAMBIOS
          + " :DCC OFFER #" + toStr(id) + " from " + client.getNick() // CAMBIOS
          + " - \"" + filename + "\" (" + toStr(ft->getFileSize()) // CAMBIOS
          + " bytes)\r\n"); // CAMBIOS
    _send(target->getFd(), ":" + _name + " NOTICE " + target->getNick() // CAMBIOS
          + " :Accept with: DCC ACCEPT " + toStr(id) // CAMBIOS
          + " | Deny with: DCC DENY " + toStr(id) + "\r\n"); // CAMBIOS
} // CAMBIOS
 // CAMBIOS
void Server::_dccAccept(Client& client, const std::vector<std::string>& params) // CAMBIOS
{ // CAMBIOS
    // DCC ACCEPT <transfer_id> // CAMBIOS
    if (params.size() < 2) // CAMBIOS
    { // CAMBIOS
        _sendReply(client.getFd(), "461", client.getNick(), // CAMBIOS
                   "DCC ACCEPT :Not enough parameters (DCC ACCEPT <id>)"); // CAMBIOS
        return; // CAMBIOS
    } // CAMBIOS
 // CAMBIOS
    int id = std::atoi(params[1].c_str()); // CAMBIOS
    if (_transfers.find(id) == _transfers.end()) // CAMBIOS
    { // CAMBIOS
        _sendReply(client.getFd(), "554", client.getNick(), // CAMBIOS
                   "DCC :Transfer #" + toStr(id) + " not found"); // CAMBIOS
        return; // CAMBIOS
    } // CAMBIOS
 // CAMBIOS
    FileTransfer* ft = _transfers[id]; // CAMBIOS
    if (ft->getReceiver() != &client) // CAMBIOS
    { // CAMBIOS
        _sendReply(client.getFd(), "554", client.getNick(), // CAMBIOS
                   "DCC :Transfer #" + toStr(id) + " is not for you"); // CAMBIOS
        return; // CAMBIOS
    } // CAMBIOS
    if (ft->getState() != PENDING) // CAMBIOS
    { // CAMBIOS
        _sendReply(client.getFd(), "554", client.getNick(), // CAMBIOS
                   "DCC :Transfer #" + toStr(id) + " is not pending"); // CAMBIOS
        return; // CAMBIOS
    } // CAMBIOS
 // CAMBIOS
    // Abrir el archivo y empezar la transferencia // CAMBIOS
    if (!ft->openFile()) // CAMBIOS
    { // CAMBIOS
        _sendReply(client.getFd(), "554", client.getNick(), // CAMBIOS
                   "DCC :Could not open file for transfer #" + toStr(id)); // CAMBIOS
        _transfers.erase(id); // CAMBIOS
        delete ft; // CAMBIOS
        return; // CAMBIOS
    } // CAMBIOS
 // CAMBIOS
    ft->setState(TRANSFERRING); // CAMBIOS
 // CAMBIOS
    // Notificar a ambos // CAMBIOS
    _send(ft->getSender()->getFd(), ":" + _name + " NOTICE " // CAMBIOS
          + ft->getSender()->getNick() // CAMBIOS
          + " :DCC :" + client.getNick() + " accepted transfer #" // CAMBIOS
          + toStr(id) + "\r\n"); // CAMBIOS
    _send(ft->getReceiver()->getFd(), ":" + _name + " NOTICE " // CAMBIOS
          + ft->getReceiver()->getNick() // CAMBIOS
          + " :DCC :Transfer #" + toStr(id) + " starting (" // CAMBIOS
          + ft->getFilename() + ", " + toStr(ft->getFileSize()) // CAMBIOS
          + " bytes)\r\n"); // CAMBIOS
 // CAMBIOS
    // Enviar el primer chunk // CAMBIOS
    ft->sendChunk(client.getFd()); // CAMBIOS
 // CAMBIOS
    if (ft->getState() == DONE) // CAMBIOS
    { // CAMBIOS
        _transfers.erase(id); // CAMBIOS
        delete ft; // CAMBIOS
    } // CAMBIOS
 ...
 // CAMBIOS
void Server::_dccDeny(Client& client, const std::vector<std::string>& params) // CAMBIOS
{ // CAMBIOS
    if (params.size() < 2) // CAMBIOS
    { // CAMBIOS
        _sendReply(client.getFd(), "461", client.getNick(), // CAMBIOS
                   "DCC DENY :Not enough parameters (DCC DENY <id>)"); // CAMBIOS
        return; // CAMBIOS
    } // CAMBIOS
 // CAMBIOS
    int id = std::atoi(params[1].c_str()); // CAMBIOS
 ...
 // CAMBIOS
    ft->setState(CANCELLED); // CAMBIOS
    _send(ft->getSender()->getFd(), ":" + _name + " NOTICE " // CAMBIOS
          + ft->getSender()->getNick() // CAMBIOS
          + " :DCC :" + client.getNick() + " denied transfer #" // CAMBIOS
          + toStr(id) + "\r\n"); // CAMBIOS
 // CAMBIOS
    _sendReply(client.getFd(), "554", client.getNick(), // CAMBIOS
               "DCC :Transfer #" + toStr(id) + " cancelled"); // CAMBIOS
 // CAMBIOS
    _transfers.erase(id); // CAMBIOS
    delete ft; // CAMBIOS
} // CAMBIOS
 // CAMBIOS
void Server::_dccCancel(Client& client, const std::vector<std::string>& params) // CAMBIOS
{ // CAMBIOS
    if (params.size() < 2) // CAMBIOS
    { // CAMBIOS
        _sendReply(client.getFd(), "461", client.getNick(), // CAMBIOS
                   "DCC CANCEL :Not enough parameters (DCC CANCEL <id>)"); // CAMBIOS
        return; // CAMBIOS
    } // CAMBIOS
 // CAMBIOS
    int id = std::atoi(params[1].c_str()); // CAMBIOS
 ...
 // CAMBIOS
    FileTransfer* ft = _transfers[id]; // CAMBIOS
    if (ft->getSender() != &client && ft->getReceiver() != &client) // CAMBIOS
    { // CAMBIOS
        _sendReply(client.getFd(), "554", client.getNick(), // CAMBIOS
                   "DCC :Transfer #" + toStr(id) + " is not yours"); // CAMBIOS
        return; // CAMBIOS
    } // CAMBIOS
 // CAMBIOS
    ft->closeFile(); // CAMBIOS
    ft->setState(CANCELLED); // CAMBIOS
 // CAMBIOS
    Client* other = (ft->getSender() == &client) ? ft->getReceiver() : ft->getSender(); // CAMBIOS
    _send(other->getFd(), ":" + _name + " NOTICE " + other->getNick() // CAMBIOS
          + " :DCC :Transfer #" + toStr(id) + " cancelled by " // CAMBIOS
          + client.getNick() + "\r\n"); // CAMBIOS
 // CAMBIOS
    _sendReply(client.getFd(), "554", client.getNick(), // CAMBIOS
               "DCC :Transfer #" + toStr(id) + " cancelled"); // CAMBIOS
 ...
 // CAMBIOS
void Server::_dccList(Client& client) // CAMBIOS
{ // CAMBIOS
    bool found = false; // CAMBIOS
    for (std::map<int, FileTransfer*>::iterator it = _transfers.begin(); // CAMBIOS
         it != _transfers.end(); ++it) // CAMBIOS
    { // CAMBIOS
        FileTransfer* ft = it->second; // CAMBIOS
        if (ft->getSender() == &client || ft->getReceiver() == &client) // CAMBIOS
        { // CAMBIOS
            _send(client.getFd(), ":" + _name + " NOTICE " + client.getNick() // CAMBIOS
                  + " :DCC #" + toStr(ft->getId()) // CAMBIOS
                  + " " + ft->getFilename() // CAMBIOS
                  + " (" + toStr(ft->getFileSize()) + " bytes)" // CAMBIOS
                  + " sender=" + ft->getSender()->getNick() // CAMBIOS
                  + " receiver=" + ft->getReceiver()->getNick() // CAMBIOS
                  + " [" + stateToStr(ft->getState()) + "]\r\n"); // CAMBIOS
            found = true; // CAMBIOS
        } // CAMBIOS
    } // CAMBIOS
    if (!found) // CAMBIOS
    { // CAMBIOS
        _send(client.getFd(), ":" + _name + " NOTICE " + client.getNick() // CAMBIOS
              + " :DCC :No active transfers\r\n"); // CAMBIOS
    } // CAMBIOS
} // CAMBIOS
 // CAMBIOS
std::string Server::toStr(int n) const // CAMBIOS
{ // CAMBIOS
    std::ostringstream oss; // CAMBIOS
    oss << n; // CAMBIOS
    return oss.str(); // CAMBIOS
} // CAMBIOS
 // CAMBIOS
std::string Server::toStr(long n) const // CAMBIOS
{ // CAMBIOS
    std::ostringstream oss; // CAMBIOS
    oss << n; // CAMBIOS
    return oss.str(); // CAMBIOS
 ...
 // CAMBIOS
std::string Server::stateToStr(TransferState s) const // CAMBIOS
{ // CAMBIOS
    switch (s) // CAMBIOS
    { // CAMBIOS
        case PENDING:      return "PENDING"; // CAMBIOS
        case TRANSFERRING: return "TRANSFERRING"; // CAMBIOS
        case DONE:         return "DONE"; // CAMBIOS
        case CANCELLED:    return "CANCELLED"; // CAMBIOS
    } // CAMBIOS
    return "UNKNOWN"; // CAMBIOS
} // CAMBIOS
#endif // CAMBIOS